...
def create_system(local_file, token, title, language, instruct, output_file):
...
if __name__ == "__main__":
    ...
    parser.add_argument("instruct", type=str, help="instruct.")
    ...
    output_path = "./projects/"+args.project+"/"+args.flowid+"/"+language
    instruct_name = instruct.replace("*","").split(" ")[0]
    output_file = output_path+"/"+instruct_name+".json"
    category_output_folder = Path(output_path) 
    os.makedirs(category_output_folder, exist_ok=True)

    create_system(local_file, token, title, language, instruct, output_file)
